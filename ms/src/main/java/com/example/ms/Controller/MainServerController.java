...
public class MainServerController
{
    ...
    public boolean registerAdmin(@RequestBody Admin admin) {
        boolean status = restTemplate.postForObject("http://localhost:8082/user/admin/register", admin, Boolean.class);
        return status;
    }

    @RequestMapping(method = RequestMethod.POST, path = "/admin/login")
    public boolean loginAdmin(@RequestBody Admin admin) {
        boolean status = restTemplate.postForObject("http://localhost:8082/user/admin/login", admin, Boolean.class);
        return status;
    }

    @RequestMapping(method = RequestMethod.POST, path = "/author/register")
    public boolean registerAuthor(@RequestBody Author author) {
        boolean status = restTemplate.postForObject("http://localhost:8082/user/admin/author/register",author, Boolean.class);
        return status;
    }

    @RequestMapping(method = RequestMethod.POST, path = "/author/login")
    public int loginAuthor(@RequestBody Author author) {
        int status = restTemplate.postForObject("http://localhost:8082/user/author/login",author, Integer.class);
        return status;
    }
    ...
    public boolean addItem(@RequestBody MCQ mcq, @PathVariable Integer authorId) {
        boolean status = restTemplate.postForObject("http://localhost:8083/instrument/add/MCQ/"+Integer.toString(authorId),mcq, Boolean.class);
        return status;
    }
    ...
    public List<MCQ> getSubject(@PathVariable String subject, @PathVariable Integer count) {
        ...
        String url;
        url = "http://localhost:8083/instrument/get/" + subject + "/" + String.valueOf(count);
